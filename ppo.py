...
def train(cfg: ExperimentConfig = None):
    ...
    while steps < cfg.total_steps:
        ...
        for _ in range(cfg.n_epochs):
            ...
            for start in range(0, len(states), cfg.batch_size):
                ...
                loss = (-torch.min(surr1, surr2).mean() + cfg.vf_coef * F.mse_loss(values, returns_t[batch_idx]) - cfg.entropy_coef * entropy.mean())
                optimizer.zero_grad()
                loss.backward()
        ...
        updates += 1
        while next_ckpt_idx < len([2000, 5000, 10000, 20000, 50000, 100000, 200000, 300000]) and episodes >= [2000, 5000, 10000, 20000, 50000, 100000, 200000, 300000][next_ckpt_idx]:
        ...
        if updates % cfg.log_interval == 0:
            ...
            print("[PPO]  step %d | ep %d | upd %d | win %.1f%% | avg_r %.3f | loss %.4f | ent@g0 %.2f | %d s/s" % (steps, episodes, updates, win_rate, avg_reward, avg_loss, entropy_g0, int(steps / elapsed)))
    ...
    print("\n  Generalisation — train: %.1f%% | test: %.1f%% | gap: %.1fpp" % (train_wr * 100, test_wr * 100, (train_wr - test_wr) * 100))
