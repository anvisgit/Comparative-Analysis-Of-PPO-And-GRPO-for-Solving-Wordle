...
from metrics import MetricsLogger, EntropyTracker, full_eval, build_frequency_tiers, win_rate_by_tier, quick_eval

EMOJI = {GREY: "⬛", YELLOW: "🟨", GREEN: "🟩"}


def print_game(env, episode_num):
    guesses = len(env.guesses_made)
    won = bool(env.guesses_made) and env.guesses_made[-1] == env.target
    display = "%d/6" % guesses if won else "X/6"
...
def collect_groups(model, env, device, cfg, entropy_tracker, adversary=None, answer_indices=None):
    ...
    for _ in range(cfg.eps_per_update):
        ...
        for group_idx in range(cfg.group_size):
            target = targets[group_idx % len(targets)]
            state = env.reset(target=target)
            trajectory_states, trajectory_actions, trajectory_logps = [], [], []
            episode_reward = 0.0
            done = False
            while not done:
                ...
                trajectory_logps.append(dist.log_prob(action).item())
                episode_reward += reward
            group_rewards.append(episode_reward)
            group_trajectories.append((trajectory_states, trajectory_actions, trajectory_logps))
        ...
        for idx, (trajectory_states, trajectory_actions, trajectory_logps) in enumerate(group_trajectories):
            advantage = float(rewards[idx])
            for state, action, logp in zip(trajectory_states, trajectory_actions, trajectory_logps):
...
def train(cfg: ExperimentConfig = None):
    ...
    words, train_answers, test_answers = get_split_words(test_fraction=cfg.test_fraction, seed=cfg.seed, vocab_type=cfg.vocab_type)
    ...
    while total_episodes < cfg.total_episodes:
        model.eval()
        states_np, actions_np, logps_np, adv_np, ep_rewards, ep_wins, adv_idx, adv_rewards = collect_groups(
            model, train_env, device, cfg, entropy_tracker, adversary=adversary, answer_indices=answer_indices
        )
        if adv_idx:
            adversary.update(adv_optimizer, adv_idx, adv_rewards)
        ...
        for _ in range(cfg.n_epochs):
            ...
            for start in range(0, len(states), cfg.batch_size):
                ...
                batch_count += 1
        while next_ckpt_idx < len([2000, 5000, 10000, 20000, 50000, 100000, 200000, 300000]) and total_episodes >= [2000, 5000, 10000, 20000, 50000, 100000, 200000, 300000][next_ckpt_idx]:
        ...
        if updates % cfg.log_interval == 0:
            ...
            print("[GRPO] ep %d | upd %d | win %.1f%% | avg_r %.3f | loss %.4f | ent@g0 %.2f | %d ep/s" % (total_episodes, updates, win_rate, avg_reward, avg_loss, entropy_g0, int(total_episodes / elapsed)))
