...
def collect_groups(model, env, device, cfg, entropy_tracker, adversary=None, answer_indices=None, episode_offset=0):
    ...
    for _ in range(cfg.eps_per_update):
        ...
        for group_idx in range(cfg.group_size):
            ...
            episode_num += 1
            if cfg.show_game_every and episode_num % cfg.show_game_every == 0:
                print_game(env, episode_num)
...
def train(cfg=None):
    ...
    while total_episodes < cfg.total_episodes:
        ...
        states_np, actions_np, logps_np, adv_np, ep_rewards, ep_wins, adv_idx, adv_rew = collect_groups(
            model, train_env, device, cfg, entropy_tracker, adversary=adversary,
            answer_indices=answer_indices, episode_offset=total_episodes
        )
        if adv_idx:
        ...
        for _ in range(cfg.n_epochs):
            ...
            for start in range(0, len(states), cfg.batch_size):
                ...
                logits, _ = model(states[batch_idx])  # ignore critic output — GRPO has no critic
                dist = torch.distributions.Categorical(logits=logits)
                new_logps = dist.log_prob(actions[batch_idx])
                # GRPO: simple policy gradient, NO clipping, NO critic loss.
                # Advantages are already group-normalised from collect_groups().
                pg_loss = -(new_logps * advantages_t[batch_idx]).mean()
                entropy_loss = -cfg.entropy_coef * dist.entropy().mean()
                # KL penalty against old policy instead of clipping
