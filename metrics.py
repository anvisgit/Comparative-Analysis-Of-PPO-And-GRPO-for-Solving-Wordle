...
def constraint_reduction_rates(env: WordleEnv) -> List[float]:
    ...
    for step_idx, (guess, feedback) in enumerate(zip(env.guesses_made, env.feedback_history)):
        guesses_so_far = env.guesses_made[: step_idx + 1]
        feedbacks_so_far = env.feedback_history[: step_idx + 1]
        new_pool = [w for w in prev_pool if is_consistent(w, guesses_so_far, feedbacks_so_far)]
        eliminated = len(prev_pool) - len(new_pool)
        rates.append(float(eliminated / max(1, len(prev_pool))))
        prev_pool = new_pool
    return rates
...
def build_frequency_tiers(answers: List[str], n_tiers: int = 3) -> Dict[str, List[str]]:
    ...
    if WORDFREQ_AVAILABLE:
        scored = sorted(answers, key=lambda w: word_frequency(w, "en"), reverse=True)
    else:
...
def win_rate_by_tier(model, env: WordleEnv, dev, tiers, test: bool = True) -> Dict:
    import torch
    model.eval()
    results = {}
    ...
    with torch.no_grad():
        for _ in range(n):
            ...
            while not done:
                ...
                logits, _ = model(s_t)
                mask = torch.tensor(env.get_valid_mask(), dtype=torch.bool, device=dev)
                masked = logits.clone(); masked[0, ~mask] = -1e9
                probs = F.softmax(masked, dim=-1)
                log_p = torch.log(probs + 1e-12)
                ent = -(probs * log_p).sum().item()
                entropy_by_pos[step_num].append(ent)
            ...
            if info["won"]:
                ...
                guess_hist[0] += 1
            ep_csr = constraint_reduction_rates(env)
            all_csr.append(ep_csr)
    mean_csr = mean_constraint_reduction(all_csr, max_steps=6).tolist()
    mean_ent = {k: float(np.mean(v)) for k, v in entropy_by_pos.items()}
...
class MetricsLogger:
    ...
    def log_final_eval(self, d: Dict):
        self.final_eval = d

    def log_tier_results(self, d: Dict):
        self.tier_results = d

    def log_hyperparams(self, **kwargs):
        self.hyperparams = kwargs

    def log_adversary_ranking(self, ranking: List):
        self.adversary_ranking = ranking
